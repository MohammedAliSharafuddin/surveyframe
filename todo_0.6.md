...
> analysis now ships first (`todo_0.5.md`, targeted within 15 days)
> because it is narrower in scope and self-contained. This release
> carries the real engineering weight of the pair (the fitting data
> contract in section 0, the semScreenR external dependency) and follows
> it. The 2027-03-11 target below predates the swap and has not been
> reset; treat it as stale pending a fresh schedule once 0.5 ships.
...
(v0.5 section — intent only, this file is the verified plan).

Last updated: 2026-08-15 (swap only; content last verified 2026-07-25).
Target CRAN submission: 2027-03-11 (stale, see note above). Theme:
...
## Hard blockers (start tracking during the 0.6 cycle)
...
1. **semScreenR is not on CRAN and not cloned locally as of 2026-07-25.**
   ...
   engagement scheduled during the 0.6 cycle, and the package submitted
...
- Plan ids `cfa_fit`, `sem_fit`, `pls_sem_fit` (and `invariance` if
  confirmed) wired through every point of the todo_0.6 integration
...
- The semScreenR `triage_apply` fix is a separate engagement in its own
  repo during the 0.6 cycle. Never fold it into this release's pool.
