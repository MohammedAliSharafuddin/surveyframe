# review_040/_setup.R
# Shared setup for every file in the 0.4.0 review suite.
# Each .qmd sources this in its first chunk. Nothing here prints.

# ---------------------------------------------------------------------------
# 1. Load the 0.4.0 build, wherever it happens to be
# ---------------------------------------------------------------------------
# DESCRIPTION still reads 0.3.4 until release task E1, so a version check
# cannot tell 0.4.0 apart from the CRAN build. Test for a 0.4.0 feature
# instead: sensitivity_analysis() is new in this release.

sf_is_040 <- function() {
  isTRUE(requireNamespace("surveyframe", quietly = TRUE)) &&
    length(getNamespaceExports("surveyframe")) > 0 &&
    "sensitivity_analysis" %in% getNamespaceExports("surveyframe")
}

if (!sf_is_040()) {
  candidates <- c(
    "../surveyframe-v0.5-dev",
    "../../surveyframe-v0.5-dev",
    "~/Documents/GitHub/surveyframe-v0.5-dev"
  )
  hit <- NULL
  for (p in candidates) {
    p <- path.expand(p)
  ...
  if (is.null(hit)) {
    stop("Cannot find the 0.4.0 source tree. Expected one of:\n  ",
