...
A branch shows or hides an item based on an earlier answer. Branch a dedicated
follow-up, not an item from a shared scale, or groups may receive
different versions of the scale being compared.
...
`validate_sframe()` returns a diagnostic. Print it to read the result, and use
the accessors to get at the parts.
...
In production, strict validation stops with a structured error when anything
is wrong. When it passes, `as_sframe()` recovers the instrument with its
validated stamp set.
