...
the main workflow using a five-item customer satisfaction instrument.
...
the visual workflow, use `launch_studio()`:
...
computing KMO adequacy and Bartlett's test. Researchers can then estimate the
factor solution with `psych::fa()` or a similar modelling function.
...
```{r report, eval = FALSE}
# Uses the internal HTML fallback unless the Quarto CLI is available
render_report(
  instrument          = instr,
  data                = responses,
...
## The main pipeline in twelve lines
