...
`topic_model_lda`, `stm_topics`) need one or more Suggests-only packages, guarded
with `rlang::check_installed()`, and every guarded section below knits
cleanly on either kind of machine.
...
real to show, where an even split would leave only a coincidence.
...
words (a built-in English list ships with the package, so this stage runs on base
surveyframe), and counts. `ngram_frequency()` does the same for
...
complaint sense of the word, where the North uses the compliment. A group split like this is what turns
...
table's `note` column, which keeps a handful of comments from reading as a
trend.
...
way to read what a keyword actually means in context, and it settles whether
a frequent term keeps one sense throughout.
...
The analysis-plan method `co_occurrence` counts how often pairs of frequent
terms appear together within the same
...
`"bing"` positive/negative lexicon, which arrives with tidytext itself. Like `term_freq`, it accepts an optional `group` role.
...
summary ahead of an analysis: feature count, sparsity, and the leading features
provide a sanity check before a heavier method.
...
example fast. A real study would try three to six values of `k` and
compare fit. `k = 3` (or `topic_model_lda`'s default `k = 4`) is a
demonstration value chosen to keep the example fast, with a fit criterion
left out of it. The accepted way to choose `k` is to fit a range of
...
same purpose for `topic_model_lda`. Both stay in their own packages.
...
algorithmic methods here, resting on human interpretation where these methods
count and cluster, and is out of scope by design. `extract_quotes()`'s output is
deliberately a plain data frame, clean enough to export and take into a
dedicated qualitative coding tool for that next step, which surveyframe
leaves to the tools built for it.
