...
declared explicitly. With `strict = TRUE`, an undeclared column is an error.
With `strict = FALSE`, it is retained with a warning. Strict mode therefore
catches an unexpected export instead of silently discarding columns.
...
```{r score-distributions, fig.width = 7, fig.height = 3, fig.align = "left", fig.alt = "Histograms of the scored scale distributions, one panel per scale"}
...
Pass `plots = TRUE` to attach a brand-styled `ggplot2` chart to each result
that supports one (descriptive, correlation, chi-square, and regression
blocks). The chart sits in `$plot` alongside the existing `$table` and
`$apa` elements.

```{r run-plots, eval = requireNamespace("ggplot2", quietly = TRUE), fig.alt = "Chart attached to the first analysis-plan result by run_analysis_plan with plots enabled"}
...
You now have imported, screened, scored, and analysed responses. Continue with
the scale reliability and validity vignette for measurement evidence, or copy
an editable worked notebook with `sframe_demo_qmd("two_group")`.
