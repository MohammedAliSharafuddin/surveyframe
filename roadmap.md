...
Current status below reflects the repository state reviewed on April 17,
2026.
...
- GitHub Actions on `main` are green for `R CMD check`, `pkgdown`, and
  `test-coverage` after the Quarto report fix shipped on April 16, 2026
  UTC.
...
- The Quarto reporting path is covered by tests and verified in local
  and CI package checks.
...
- The release surface is stable enough for a serious CRAN submission
  pass.
