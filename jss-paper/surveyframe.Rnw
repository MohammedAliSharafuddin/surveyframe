...
\author{Mohammed Ali Sharafuddin\\Independent Researcher\\ORCID: 0000-0001-5247-2964}
...
\Address{
  ...
  Email: \email{mohammedali.page@gmail.com}\\
  URL: \url{https://github.com/MohammedAliSharafuddin/surveyframe}
}
...
analysis plan---from the instrument, not from the data.
...
\begin{description}
  ...
  \item[\code{sf\_item()}] Defines a single survey item. Supports
    thirteen item types: \code{"likert"}, \code{"single\_choice"},
    \code{"multiple\_choice"}, \code{"numeric"}, \code{"text"},
    \code{"textarea"}, \code{"date"}, \code{"matrix"}, \code{"slider"},
    \code{"ranking"}, \code{"rating"}, \code{"section\_break"}, and
    \code{"text\_block"}.
...
analysis plan---produces a hash mismatch, which is reported as a
warning.
...
\code{survey\_module\_ui()} and \code{survey\_module\_server()}
provide a reusable module that embeds a survey within a larger
\pkg{Shiny} application, returning a reactive holding the response
list when submitted.
...
<<read-responses, eval=FALSE, echo=TRUE>>=
...
and Bartlett's test of sphericity via \code{psychometrics()}:

\begin{Code}
R> if (requireNamespace("psych", quietly = TRUE)) {
+    pm <- psychometrics(resp, instr)
+    print(pm)
+  }
\end{Code}

The function constructs the correlation matrix from scored item
columns identified in the instrument and passes it to
\code{psych::KMO()} and \code{psych::cortest.bartlett()}, ensuring
only the declared measurement items are included.
...
string, and a \code{prompt} field---a plain-language interpretation
template that the researcher completes:

<<analysis-plan, echo=TRUE>>=
...
the internal \code{.sframe\_citations} registry. When the ASRDA
textbook \citep{sharafuddin2026asrda} is published, \pkg{sfReport}
(a planned Phase~4 companion package) will extend the citation block
to reference the specific ASRDA chapter corresponding to each analysis
step.
...
\pkg{surveyframe} targets PLS-SEM through the planned \pkg{sfSEM}
companion package.
...
include \pkg{sfReport} (Quarto-native report generation with
defensibility appendices referencing the ASRDA methodological
textbook), \pkg{sfSEM} (PLS-SEM and CB-SEM execution via
\pkg{lavaan} and \pkg{seminr}), \pkg{sfIRT} (item response theory via
\pkg{mirt}), and \pkg{sfMCDM} (multi-criteria decision making). The
development roadmap is maintained publicly at
\url{https://github.com/MohammedAliSharafuddin/surveyframe-dev}.
