...
sf_indirect <- function(from, through, to, label = NULL) {
  structure(
    list(from = from, through = as.character(through), to = to, label = label),
    class = "sf_indirect"
  )
}
...
validate_model <- function(model, instrument = NULL, strict = TRUE) {
  ...
  for (con in constructs) {
    ...
    if ((model$type %||% "") %in% c("cfa", "cb_sem") && mode %in% c("formative", "composite")) {
      problems <- c(problems,
        paste0("lavaan model '", model$id, "' cannot use construct '", con$id,
               "' with mode '", mode, "' in v0.3 syntax generation."))
    }
  }
  ...
  for (path in paths) {
    if (!path$from %in% construct_ids) {
      problems <- c(problems,
        paste0("Path source '", path$from, "' is not a construct."))
    }
    if (!path$to %in% construct_ids) {
      problems <- c(problems,
        paste0("Path target '", path$to, "' is not a construct."))
    }
...
sem_lavaan_syntax <- function(model, instrument = NULL, standardised = TRUE) {
  ...
  if (length(paths) > 0) {
    ...
    for (target in names(incoming)) {
      rhs <- vapply(incoming[[target]], function(path) {
        if (!is.null(path$label) && nzchar(path$label)) {
          paste0(path$label, "*", path$from)
        } else {
          path$from
        }
      }, character(1))
  ...
  if (length(indirect) > 0) {
    ...
    for (ind in indirect) {
      ...
      effect_name <- ind$label %||% paste0("indirect_", sframe_lavaan_path_label(ind$from, ind$through, ind$to))
