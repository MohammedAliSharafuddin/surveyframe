make_v03_instrument <- function(reverse = FALSE) {
  ...
  gen <- sf_item(
    ...
    "Gender?",
    type = "single_choice",
    choice_set = "gender_choices"
  )

  attn <- sf_item(
    "attn_q",
    "Please select Agree (4).",
    type = "likert",
    choice_set = "ag5"
  )
...
make_v03_responses <- function(n = 50, seed = 42) {
  ...
  data.frame(
    ...
    submitted_at = as.character(
      base_time - sample(100:3600, n, replace = TRUE)
    ),
...
test_that("descriptives_report computes core summaries and groups", {
  ...
  expected_columns <- c(
    "valid_n", "missing_n", "mean", "sd", "median", "iqr",
    "skewness", "kurtosis", "se", "ci_low", "ci_high"
  )

  expect_true(all(expected_columns %in% names(dr$table)))

  expect_equal(
    dr$table$missing_n[dr$table$variable == "x" & dr$table$group == "b"],
    1
  )
})
...
test_that("missing_data_report returns item, respondent, pattern, and scale rules", {
  ...
  expected_sections <- c(
    "item_missing", "respondent_missing", "patterns",
    "deletion", "scale_missing_rules", "mcar"
  )

  expect_true(all(expected_sections %in% names(mr)))
...
test_that("outlier_report flags univariate and multivariate outliers", {
  ...
  expect_s3_class(iqr, "sframe_outlier_report")
...
test_that("model layer constructors reject invalid identifiers", {
  expect_error(
    sf_construct("bad id", items = "sat_1"),
    class = "sframe_validation_error"
  )

  expect_error(
    sf_construct("SAT", items = "bad item"),
    class = "sframe_validation_error"
  )
  ...
  expect_error(
    sf_covariance("SAT", "bad-id"),
    class = "sframe_validation_error"
  )

  expect_error(
    sf_indirect("SAT", c("MED", NA_character_), "LOY"),
    class = "sframe_validation_error"
  )

  expect_error(
    sf_model("bad model", type = "cfa"),
    class = "sframe_validation_error"
  )
})
...
test_that("validate_sframe catches invalid model and analysis references", {
  ...
  bad_model <- sf_model(
    ...
    type = "cfa",
    constructs = list(
      sf_construct("BAD", items = c("sat_1", "ghost"))
    )
  )
...
test_that("builder registry declares roles and conditional significance level", {
  html <- paste(
    readLines(launch_builder(open = FALSE), warn = FALSE),
    collapse = "\n"
  )
...
test_that("sample_size_plan and validity_report return structured outputs", {
  ...
  vr <- validity_report(list(
    SAT = c(sat_1 = 0.70, sat_2 = 0.80, sat_3 = 0.75)
  ))
...
test_that("ordinal and multinomial logistic runners work when optional packages exist", {
  skip_if_not_installed("MASS")
  skip_if_not_installed("nnet")

  dat <- data.frame(
    y_ord = ordered(rep(c("low", "mid", "high"), each = 10)),
    y_nom = factor(rep(c("a", "b", "c"), each = 10)),
    x = rep(seq_len(10), 3)
  )

  ord <- surveyframe:::sframe_run_ordinal_logistic(
    dat,
    list(dependent = "y_ord", predictors = "x")
  )

  multi <- surveyframe:::sframe_run_multinomial_logistic(
    dat,
    list(dependent = "y_nom", predictors = "x")
  )

  expect_type(ord, "list")
