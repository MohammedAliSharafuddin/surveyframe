...
- [x] RStudio is open with a clean R session (restart R before starting).
- [x] A modern browser is available (Chrome or Firefox).
- [x] `devtools`, `remotes`, `psych`, `rmarkdown`, `shiny`, and `googlesheets4`
  are installed (the last two can be absent — they are Suggests only).
- [x] Internet access is available for the GitHub install step.
...
- [x] Installation completes with no errors.
- [x] No warning about missing Imports packages (jsonlite, rlang, openssl must
...
- [x] The function list is non-empty and includes `sf_instrument`, `sf_item`,
...
- [x] No startup messages or warnings.
...
- [x] Title reads: `surveyframe: Survey Instrument Workflows` (no "for R").
- [x] The note field shows `R package version 0.3.2`, not the old hard-coded `0.3.0`.
- [x] Author, ORCID, and URL are correct.
...
- [x] Each help page opens without error.
- [x] Examples in each page are present and readable.
...
- [x] Each call returns at least `format.<class>`, `print.<class>`, `summary.<class>`.
...
- [x] `print(cs)` shows a header line and a two-column value/label table.
- [x] `format(cs)` returns a compact single character string.
- [x] `summary(cs)` shows `allow_other` and `randomise` flags.
...
- [x] `print(it)` shows id, type, label, choice set.
- [x] `summary(it)` shows required and reverse flags.
...
- [x] `print(sc)` shows id, item count, items list, and scoring method.
- [x] `format(sc)` says "3 item(s)".
...
- [x] `print(br)` shows the rule in readable English: "show when q1 == yes".
- [x] No error about missing or wrong field names.
...
- [x] `print(ck)` shows id, type, item_id, and pass values.
- [x] `summary(ck)` shows `fail_action`.
...
- [x] `print(mdl)` shows id, type, 0 construct(s), label, and engine.
- [x] `summary(mdl)` shows engine and structural path count.
...
- [x] `print(instr)` shows a tidy summary: title, item count, scale count,
  check count, and validation status.
- [x] No error during construction.
- [x] The logo downloads without error and
  `nchar(instr$render$header$logo_base64)` is a few thousand characters.
- [x] The institution, logo, and `#16B3B1` theme appear on the exported survey
...
- [x] `v$valid` is `TRUE`.
- [x] `v$problems` is empty (length 0) or contains only expected informational notes.
...
- [x] `length(instr$models)` is 1 after `add_model()`.
- [x] You understand that the analysis plan is declared in the builder Analyse
...
- [x] Both `identical()` calls return `TRUE`.
- [x] No hash mismatch error on `read_sframe()`.
...
- [x] An error is caught mentioning hash or integrity.
...
- [x] The builder loads without a blank screen or JavaScript errors (open the
  browser console — no red errors).
- [x] You can add a new question using the Add Item button.
- [x] The item type dropdown includes all 13 types: Likert, Single choice,
  Multiple choice, Matrix, Numeric, Text, Long text, Date, Slider, Rating,
  Ranking, Section break, Text block.
- [x] You can add a choice set and assign it to a Likert item.
- [x] You can add a scale and assign items to it.
- [x] The Save .sframe button downloads a `.sframe` file.
- [x] The Open button accepts the saved file and restores the instrument.
- [x] The Analyse tab shows three distinct stages with captions: Plan, Run
  preview (drops syntax-only methods), and Report outline (adds the models
  section). They do not show identical content.
- [x] You can declare an analysis plan and drag plan cards to reorder them on
  the Plan tab. This is where the analysis plan is authored as part of design.
- [x] The Export survey button downloads a deployable HTML survey.
- [x] Survey settings > Google Sheets has a Generate collector (.gs) button and
  numbered deploy steps. The settings gear button is clearly visible.
- [x] No broken images, garbled text, or mojibake anywhere in the UI.

### Step F3 — Exported output from the builder

- [x] Open the survey HTML exported in Step F1 (Export survey). It opens as a
...
- [x] The builder opens pre-loaded with the demo instrument.
- [x] Items, choice sets, and scales are all visible without a manual load step.
...
- [x] Shiny app launches in the browser with the demo instrument and 120
  responses already loaded.
- [x] The left navigation shows eight screens, in order: Open Instrument,
  ...
  the builder).
- [x] The logo and brand colours display correctly. No R console error.
...
- [x] Open Instrument loads a `.sframe` (or the demo is already loaded). The hint
  says design happens in the SurveyBuilder.
- [x] Preview Survey shows the EXACT deployable survey inside a frame, with the
...
- [x] Upload Responses has a "Load sample survey and responses" button and a
  "Download sample CSV" link.
- [x] Quality Dashboard, Reliability, and Analysis Plan all render content (not
  blank) once data is loaded.
- [x] Analysis Plan shows three stages: Plan, Run preview, Report outline. The
  variables panel excludes section breaks and text blocks. The 36 demo plans
  appear in the saved-plans table.
- [x] Run preview executes the data-driven plans and excludes the syntax-only
  methods with a note.

### Step G4 — Integrated dashboard and export

- [x] The Dashboard screen shows Overview, Items, Scales, and Data inline (KPIs,
  item bar charts, scale histograms, a raw-data table). Charts render, not blank.
- [x] The Export screen offers Download .sframe and Generate HTML report only,
...
- [x] File is written without error.
- [x] A message confirms the path and size in KB.
- [x] The browser opens automatically.

### Step H2 — Header and welcome screen

In the browser:

- [x] The header band shows the GitHub logo set in Step D1 and the institution
  name "surveyframe demo". The logo is sized consistently (not stretched).
- [x] The brand theme colour (`#16B3B1`) is applied to the accent and buttons.
- [x] A welcome screen appears before any questions.
- [x] The survey title "MAS Review Test Survey" is visible.
- [x] A Start button is present.

### Step H3 — Question rendering

Click Start and review the question pages:

- [x] The Likert item (`q1`) renders with options in a horizontal row (one
  column per option, label below each radio button).
- [x] The second Likert item (`q2`) renders in the same horizontal layout.
- [x] The attention check item (`attn_q`) — declared as `single_choice` but
...
- [x] The required marker (`*`) appears next to all required items.
- [x] This minimal instrument is a single page, so NO progress bar shows (the
...
- [x] A "Built with surveyframe" footer appears at the foot of the survey.

### Step H4 — Validation and submission

- [x] Clicking Next without answering a required item shows an inline error
  below the item AND a page-level banner at the top of the card.
- [x] Answering the items and clicking Submit downloads a CSV file.
- [x] Open the downloaded CSV: it has a `respondent_id` column and one column
  per item.
...
- [x] All items are readable on a narrow screen without horizontal scrolling.
- [x] The Likert row wraps gracefully on very small screens (360px wide).
...
- [x] The app launches and the survey renders inside the page.
- [x] Completing and submitting the survey fires the `observeEvent` notification.
...
- [x] Counts are non-zero and match the tourism demo (15 items, 5 scales, 120
...
- [x] Returns a one-row data frame.
- [x] No error. (Without declaring `respondent_id` and `started_at`, strict mode
...
- [x] `$n_respondents` matches the number of rows in `resp`.
- [x] `print(qr)` shows a readable summary — no raw list output.
- [x] No error about attention check items.
...
- [x] `scored` has all original columns plus one per scale.
- [x] Scale means are between 1 and 5 (Likert range).
- [x] The data frame prints with Scale, Mean, and SD columns.
...
- [x] Prints a summary of missing values per item.
- [x] `mcar` slot returns `available = FALSE` (Little's MCAR is not implemented
...
- [x] One reliability row per scale.
- [x] Alpha values are between 0 and 1.
- [x] No psych internal warnings printed to the console.
...
- [x] Returns item-level statistics (mean, SD, skewness for each item).
...
- [x] KMO overall value is printed.
- [x] Bartlett's test chi-square and p-value are printed.
- [x] Suggested number of factors is printed.
- [x] No errors.
...
- [x] CR and AVE are computed for each construct.
- [x] AVE is between 0 and 1.
- [x] No error.
...
- [x] Normality results are shown per scale variable.
- [x] No error.
...
- [x] `cfa_lavaan_syntax()` returns a multi-line lavaan model string
  (e.g., `digital_marketing =~ dm_1 + dm_2 + ...`).
- [x] `sem_lavaan_syntax()` includes the measurement model and the declared paths.
- [x] `seminr_syntax()` returns the seminr measurement and structural model syntax.
- [x] None of these calls require lavaan or seminr to be loaded.
...
- [x] Returns a list with one element per declared analysis plan (the tourism
...
- [x] No `Error:` line appears for any plan (all runners now resolve their role
...
- [x] `$apa` is a formatted string (e.g., `r(60) = 0.43, p = .002`).
- [x] `$prompt` is a complete sentence template.
- [x] `$test` names the statistical method used.
- [x] No `$table`, `$plot`, or `$reference` field is present at the runner level
...
- [x] `nrow(cb$items_table)` equals the number of items.
- [x] The first 6 rows show correct ids and types.
- [x] No error. Note: `codebook_report()` has no `path` argument — it returns
...
- [x] File is written without error.
- [x] `file.size()` returns a positive number.
- [x] Open the HTML: each research question has its own section with the APA
  statistic, effect size, and writing prompt.
- [x] The results table is academic style (horizontal rules only) and the
  accent and footer are on-brand (teal, "Built with surveyframe").
- [x] The References section cites surveyframe with the current title
...
- [x] File is written without error. `render_report()` renders through Quarto
...
- [x] Open the HTML: sections appear in order (codebook, quality, missing data,
  descriptives, response distributions, reliability, analysis results).
- [x] A "Response distributions" section shows one chart per item (bar chart for
  categorical, histogram for numeric) and one histogram per scale.
- [x] No raw markdown or raw R output leaks in (all tables render as HTML tables,
  none show pipe characters).
- [x] Tables are padded and readable; wide tables scroll within the column rather
  than overflowing; numeric values are rounded to two decimal places.
- [x] Reference titles render in italics, not as literal `*asterisks*`, and the
  surveyframe reference shows the current title and version.
- [x] `cat()` prints the report path and size in KB (as in Step Q1).
...
- [x] Dashboard opens in the browser automatically.
- [x] Five tabs are visible: Overview, Items, Scales, Quality, Raw data.
- [x] Overview shows response count, date range, and instrument metadata.
- [x] Items tab shows per-item charts. Charts appear when you click the tab
  (they do not stay blank).
- [x] Scales tab shows scale score distributions. Charts appear when you click.
- [x] Quality tab shows attention-check pass rates.
- [x] Raw data tab shows a scrollable table with a CSV download button.
...
- [x] `lavaan` appears in Suggests.
- [x] `lavaan` does NOT appear in Imports.
- [x] The three hard Imports are exactly: `jsonlite`, `rlang`, `openssl`.
...
- [x] No `str()` calls anywhere in the file.
- [x] `export_static_survey()` call is present and inside `if (interactive()) { }`.
- [x] `render_results()` call is present and inside `if (interactive()) { }`.
- [x] Every other browser-opening or Shiny call (if any) is also guarded.
...
- [x] The `export_static_survey()` guard fires and prints "Static survey written to: ..."
  plus the file size.
- [x] The `render_results()` guard fires and prints "Report written to: ..." plus
  the file size.
- [x] `sessionInfo()` prints at the end.
...
- [x] Knit completes with no errors. Warnings from psych internals are expected
  and acceptable.
- [x] Open the output HTML in a browser.

Work through the vignette output from top to bottom:

#### Score and describe section

- [x] A note below `score_scales()` explains that pre-scored columns are skipped.
- [x] Scale means display as a labelled data frame with Scale, Mean, and SD
  columns — not as a raw named numeric vector.

#### Reliability section

- [x] The interpretation note explains when to prefer omega over alpha:
...
- [x] This section exists between Reliability and Analysis Plan.
- [x] `efa_report()` is called inside a `psych` availability check.
- [x] KMO threshold (0.60) and Bartlett's test are mentioned in plain language.

#### Construct validity section

- [x] This section follows EFA readiness.
- [x] `validity_report()` is called with a named loadings list.
- [x] CR and AVE thresholds are stated (AVE above 0.50, CR above 0.70).
- [x] The Fornell-Larcker criterion is explained in one sentence.

#### Assumption checks section

- [x] This section appears between the Analysis Plan and Run the Analysis Plan.
- [x] `assumption_report()` is called and its output is printed.
- [x] A note explains how to use normality and homogeneity results to choose
  between parametric and non-parametric tests.

#### Results report section

- [x] After `render_results()`, the code shows both the file path and the file
...
- [x] All six supporting vignettes knit without errors.
- [x] `surveybuilder-gui-overview.Rmd` describes eight studio screens (no Build
  Survey screen) and the builder export buttons.
- [x] `deploying-and-collecting.Rmd` (new) covers the Apps Script collector,
...
- [x] All 368 tests pass (FAIL 0, WARN 0, SKIP 0, PASS 368).
- [x] No new unexpected warnings in the test output.
...
- [x] Every exported function that receives a non-sframe object returns a clear,
  actionable error message pointing the user to `sf_instrument()` or
  `read_sframe()`. No raw `inherits()` assertion failures.
- [x] No output from any function goes to the console unintentionally (psych
  suppressions are in place, run_analysis_plan is silent on assignment).
- [x] The static survey works on a phone-sized screen (Step H5).
- [x] The dashboard charts actually render when their tab is clicked, not blank.
- [x] The S3 methods for all 6 sub-classes produce readable, professional output
  that you would be comfortable showing in a JSS article.
- [x] The CITATION is correct and will display correctly on the CRAN page after
...
### Full sign-off (2026-06-17)

- **Parts A through U: SIGNED OFF.** Every checklist item above is ticked.
  Reviewed against the freshly installed 0.3.2 from the public repo. Covers
  installation, citation, the six S3 methods, instrument construction (header,
  logo, and brand theme in Step D1), serialisation and hash integrity, the
  SurveyBuilder (distinct Analyse stages, Export survey and Generate collector
  buttons, footer, academic tables), the SurveyStudio (eight screens, no Build
  screen, iframe Preview, integrated Dashboard with brand pill tabs and academic
  tables, Quality elaborated, brand teal, alignment fix, Export buttons disabled
  until an instrument is valid, and Google Sheet response import on the Upload
  screen), the static survey export with its header logo, the analysis plan and
  runners, the reporting routes (render_results and render_report sharing one
  branded template), the standalone dashboard, the syntax generators, and the
  vignettes (analysis output now shown as branded tables and plots).
- **Fixes confirmed during review:** the `sprintf` warning in
  `R/survey_module.R`, dashboard logo serving (`addResourcePath`),
  `launch_dashboard()` no longer silently loading demo data, partial-correlation
  and mediation runner role aliases, the numeric moderation moderator in the
  demos, psych console noise silenced, report styling unified, the `replicate.R`
  `output_path` argument, vignette prose semicolons removed, and the test count
  corrected to 368.

The .bib reference refactor is deferred to v0.3.4 and recorded in
`revision_todo_0.3.md`.

**Reviewed by:** Mohammed Ali Sharafuddin  
**Date signed off:** 2026-06-17 (Parts A through U, complete)  
**All steps complete:** Yes. All 184 checklist items ticked.  
**Notes for the developer (if any):** Proceed to R CMD check and CRAN submission.
